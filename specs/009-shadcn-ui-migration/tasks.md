...
>
> **Follow-up — DONE.** The 3 obsolete spec files (`adhoc-product`, `catalog-selector`, `product-details`) were rewritten to match current behavior (unified product combobox + click/keyboard product-details Dialog). Unused generated `ui` primitives (`card`, `collapsible`, `separator`, `command`, `popover`) and the now-unused `cmdk` dependency were removed. **Full E2E suite is now 68/68 green; `npm test` 185/185.**
