...
test.describe('Feature 007 — Order filters', () => {
  ...
  test('Scenario 1: client name search filters and clears', async ({ page }) => {
    ...
    await expect(page.locator('.order-list-empty')).toContainText('Nicio comandă găsită');
  ...
  test('Scenario 6: combined multi-filter + reset button', async ({ page }) => {
    ...
    await expect(page.locator('.order-filters__reset')).toHaveCount(0);
    ...
    await page.locator('.order-filters__reset').click();
    await expect(page.locator('.order-row')).toHaveCount(3);
